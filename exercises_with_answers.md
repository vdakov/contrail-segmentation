...
**Answer:**  
The augmented loss is much less stable than the non-augmented one. This makes sense as augmentations change over forward passes and the task is overall mcuh harder.
</div>
...
```python
transform_aug_contrails = A.Compose([
...
```python
train_contrail_network(transform_no_aug_contrails, EPOCHS, "focal", "base", dataset="own", experiment_name="no_augmentation-focal")
```

```python
train_contrail_network(transform_no_aug_contrails, EPOCHS, "sr", "base", dataset="own", experiment_name="no_augmentation-sr")
```
...
```python
train_contrail_network(transform_contrails_aug, EPOCHS, "focal", "base", dataset="own", experiment_name="augmentation-focal")
```

```python
train_contrail_network(transform_contrails_aug, EPOCHS, "sr", "base", dataset="own", experiment_name="augmentation-sr")
```

```python
from evaluate import evaluate_contrail_model
a = A.Compose([
            A.Resize(128, 128),
            A.Normalize(mean=0.5, std=0.5, max_pixel_value=255.0),
        ])

evaluate_contrail_model("data/models/dice:base-100epoch.torch", dataset="own", augmentation=a, num_images=4)
```
